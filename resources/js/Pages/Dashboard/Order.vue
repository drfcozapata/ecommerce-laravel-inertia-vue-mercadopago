    ...
    <DashboardLayout>
        ...
        <table class="w-full">
            <tr class="w-full bg-white border-b">
                ...
                <th class="text-left font-medium text-xs px-4 py-3">
                    Purchased on
                </th>
                <th class="text-right font-medium text-xs px-4 py-3">Total</th>
                <th class="text-center font-medium text-xs px-4 py-3">
                    Status
                </th>
            ...
            >
                ...
                <td class="px-4 py-3 text-left">
                    <span class="text-xs text-zinc-500">{{
                        $date(order.created_at).format("YYYY-MM-DD HH:mm")
                    }}</span>
                </td>
                ...
                <td class="px-4 py-3 text-center">
                    <span
                        :class="{
                            'bg-c-green-100 text-c-green-600':
                                order.status === 'Paid',
                        }"
        ...
        <div
            ...
            :class="{
                'translate-x-full': Object.keys(selectedOrder).length === 0,
            }"
        >
            <header class="mb-6">
                <span class="font-medium block"
                    >Order number: {{ selectedOrder.id }}</span
                >
                <span class="text-xs text-zinc-500 -mt-0.5 block">{{
                    $date(selectedOrder.created_at).format("YYYY-MM-DD HH:mm")
                }}</span>
            </header>
            ...
            <div
                class="bg-zinc-100 rounded p-4 flex items-center justify-between mt-4"
            >
